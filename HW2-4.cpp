...
//关键的重点：（1）函数的设置和使用；（2）字符数组的cin与cout语法；（3）字符数组一开始的长度设置；（4）字符与数值的转化
#include <iostream>
...
int main(){
    ...
    cin>>date;//这种输入是可以的。
    //char year1[4]={date[0],date[1],date[2],date[3]}; 这样初始化似乎有问题？？？
    ...
    //将日期转化为数值
    //atoi函数可以自动识别字符数组中的第一个非零元素为起点，然后遍历到\0处，同时可以忽略所有的无关元素。
    year=atoi(year1);//年数值
